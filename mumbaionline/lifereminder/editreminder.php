...
if($_POST['MM_Insert']==1)
	{
		...
		if(! $msg)
		{
			...
			$db = new db;
			$db->phpedit("lifereminder_reminder","reminder_id",$_POST,$_POST['reminder_id']);
...
$query_rsedit = sprintf("SELECT * FROM lifereminder_reminder WHERE reminder_id = %s", $colname_rsedit);
...
<?php
if($_GET['remove']==1) {
	@unlink("../uploadDir/lifereminder/".$row_rsedit['file']);
	$sql = "update lifereminder_reminder set file = '' where reminder_id = '".$row_rsedit['reminder_id']."'";
	mysql_query($sql) or die('error');
	header("Location: ".$_SERVER['PHP_SELF']."?reminder_id=".$row_rsedit['reminder_id']);
	exit;
}
?>
...
<!-- InstanceBeginEditable name="EditRegion3" -->
		  <div class="post">
            <h1 class="title"><a href="#">Edit Reminder</a></h1>
                ...
                <table border="1" cellspacing="0" cellpadding="5">
                  <tr>
                    ...
                    <td valign="top"><select name="account" id="account">
                      <option value="Bank account" <?php if (!(strcmp("Bank account", $row_rsedit['account']))) {echo "selected=\"selected\"";} ?>>Bank account</option>
                      <option value="Policy" <?php if (!(strcmp("Policy", $row_rsedit['account']))) {echo "selected=\"selected\"";} ?>>Policy</option>
                      <option value="Wills" <?php if (!(strcmp("Wills", $row_rsedit['account']))) {echo "selected=\"selected\"";} ?>>Wills</option>
                      <option value="Credit" <?php if (!(strcmp("Credit", $row_rsedit['account']))) {echo "selected=\"selected\"";} ?>>Credit</option>
                      <option value="Assets" <?php if (!(strcmp("Assets", $row_rsedit['account']))) {echo "selected=\"selected\"";} ?>>Assets</option>
                      <option value="Others" <?php if (!(strcmp("Others", $row_rsedit['account']))) {echo "selected=\"selected\"";} ?>>Others</option>
                      <option value="Message" selected="selected" <?php if (!(strcmp("Message", $row_rsedit['account']))) {echo "selected=\"selected\"";} ?>>Message</option>
                      </select>
                    </td>
                  </tr>
                  <tr>
                    <td valign="top">Account no: </td>
                    <td valign="top"><input name="accountno" type="text" id="accountno" value="<?php echo $row_rsedit['accountno']; ?>" /></td>
                  </tr>
                  <tr>
                    <td valign="top">Description:</td>
                    <td valign="top"><textarea name="description" rows="5" id="description"><?php echo $row_rsedit['description']; ?></textarea></td>
                  </tr>
                  <tr>
                    <td valign="top">Contact:</td>
                    <td valign="top"><input name="contact" type="text" id="contact"value="<?php echo $row_rsedit['contact']; ?>" /></td>
                  </tr>
                  <tr>
                    <td valign="top">Place:</td>
                    <td valign="top"><input name="place" type="text" id="place" value="<?php echo $row_rsedit['place']; ?>" /></td>
                  </tr>
                  <tr>
                    <td valign="top">File:</td>
                    <td valign="top"><?php if($row_rsedit['file'])
					{
					?>
					<a href="../uploadDir/lifereminder/<?php echo $row_rsedit['file']; ?>"><?php echo $row_rsedit['file']; ?></a> <a href="<?php echo $_SERVER['PHP_SELF']; ?>?reminder_id=<?php echo $row_rsedit['reminder_id']; ?>&remove=1">remove</a>
					<?php
					} else {
                  ...
                  <tr>
                    ...
                    <td valign="top"><select name="friendid" id="friendid">
                        <option value="" <?php if (!(strcmp("", $row_rsedit['friendid']))) {echo "selected=\"selected\"";} ?>>select friend</option>
                      <?php
do {  
?>
                        <option value="<?php echo $row_rsFriends['friendid']?>"<?php if (!(strcmp($row_rsFriends['friendid'], $row_rsedit['friendid']))) {echo "selected=\"selected\"";} ?>><?php echo $row_rsFriends['name']?></option>
                        <?php
...
?>
                  ...
                  <tr>
                    <td valign="top">&nbsp;</td>
                    <td valign="top"><input type="submit" name="Submit" value="Update Reminder" />
                      <input name="MM_Insert" type="hidden" id="MM_Insert" value="1" />
                        <input name="file" type="hidden" id="file" />
                    <input name="reminder_id" type="hidden" id="reminder_id" value="<?php echo $row_rsedit['reminder_id']; ?>" /></td>
                  </tr>
                </table>
              </form>  
	        </div>
