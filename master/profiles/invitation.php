...
$colname_rsCheck = "-1";
if (isset($_GET['invited_to_id'])) {
  $colname_rsCheck = (get_magic_quotes_gpc()) ? $_GET['invited_to_id'] : addslashes($_GET['invited_to_id']);
}
mysql_select_db($database_conn, $conn);
$query_rsCheck = sprintf("SELECT * FROM invitations WHERE invited_to_id = %s AND user_id = %s", $colname_rsCheck,$colid_rsCheck);
...
if (isset($_GET['invited_to_id'])) {
  $colname_rsFriendlist = (get_magic_quotes_gpc()) ? $_GET['invited_to_id'] : addslashes($_GET['invited_to_id']);
}
mysql_select_db($database_conn, $conn);
$query_rsFriendlist = sprintf("SELECT * FROM friends WHERE friend_id = %s AND user_id = %s", $colname_rsFriendlist,$colid_rsFriendlist);
...
if($_SESSION['user_id']==$_GET['invited_to_id']) {
	$message = "You cannot invite yourself.";
} else if($totalRows_rsCheck==0) {
	if($totalRows_rsFriendlist==1) {
		$message = "This user is already in your friendslist so you cannot send invitation to this user.";
	} else {
		$record['invited_to_id'] = $_GET['invited_to_id'];
		$record['user_id'] = $_SESSION['user_id'];
		$record['invitation_date'] = date('Y-m-d H:i:s');
		include("../Classes/db.php");
		$db = new db;
		$db->phpinsert("invitations","invite_id",$record);
